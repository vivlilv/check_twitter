...
class AccountTracker:
    ...
    def __init__(self, tracker_file="account_history.json"):
        ...
        self.lock = (
            asyncio.Lock()
        )  # Add a lock to prevent race conditions when multiple instances write to the file
    ...
    def _load_history(self) -> Dict:
        ...
        if os.path.exists(self.tracker_file):
            try:
                ...
                logger.warning(
                    f"Error reading account history file. Creating new history."
                )
                return {}
        return {}
...
class Dusted:
    ...
    async def login(self) -> Dict:
        ...
        try:
            ...
            if hasattr(self.config, "SETTINGS") and getattr(
                self.config.SETTINGS, "CHECK_TIME_RULES", True
            ):
                last_run = account_tracker.get_last_run(self.account.address)
                if last_run and not account_tracker.should_process_account(
                    self.account.address
                ):
                    logger.info(
                        f"[{self.account_index}] Skipping account - last run on {last_run} (less than 24 hours ago)"
                    )
            ...
            if not cf_token:
                raise Exception(
                    "Failed to fetch CAPTCHA token (cf). Ensure Solvium API integration is working."
                )

            json_data["cf"] = cf_token
            logger.success(
                f"[{self.account_index}] Successfully obtained CAPTCHA token"
            )
            ...
            # Get status code
            status = getattr(
                sign_in_response, "status", getattr(sign_in_response, "status_code", 0)
            )
    ...
    async def solve_captcha_with_api(self) -> Optional[str]:
        ...
        try:
            ...
            sitekey = (
                "0x4AAAAAABagd0NGZKGskHs5"  # Replace with the actual sitekey for Dusted
            )
            pageurl = "https://www.dusted.app/"

            logger.debug(
                f"[{self.account_index}] Solving CAPTCHA with sitekey: {sitekey}"
            )
            ...
            if token:
                logger.success(
                    f"[{self.account_index}] Successfully obtained CAPTCHA token: {token[:10]}..."
                )
                return token
            else:
                logger.error(
                    f"[{self.account_index}] Failed to obtain CAPTCHA token via Solvium API"
                )
                return None

        except Exception as e:
            logger.error(
                f"[{self.account_index}] Error while solving CAPTCHA with API: {str(e)}"
            )
            return None
    ...
    async def authorize_twitter(
        ...
        try:
            ...
            response = await self.session.get(
                redirect_url, params=params, headers=headers
            )
            ...
            response = await self.session.get(
                "https://www.dusted.app/rewards", params=params, headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"[{self.account_index}] finish code {response.status_code}"
                )
            ...
            try:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if data and data.get("remainingPlays") is not None:
                            games = int(data["remainingPlays"])
                            break  # Success
                    except Exception as e:
                        print(f"[Attempt {attempt+1}] Failed to fetch lasso score: {e}")
                ...
                for attempt in range(games):
                    logger.info(
                        f"[{self.account_index}] Sending lasso play request ({attempt+1})"
                    )

                    # Get auth headers and log them for debugging
                    auth_headers = self.get_auth_headers()
                    logger.debug(
                        f"[{self.account_index}] Using auth headers: {auth_headers}"
                    )
                    ...
                    if (
                        "message" in play_data
                        and "connect your Twitter" in play_data["message"]
                    ):
                        if attempt < max_retries - 1:
                            wait_time = 5  # Increasing delay between retries
                            logger.warning(
                                f"[{self.account_index}] Twitter connection not recognized. Retrying in {wait_time}s..."
                            )
                            ...
                            logger.error(
                                f"[{self.account_index}] Twitter connection issue persists after {max_retries} attempts"
                            )
                            return 0

                    successful = (
                        True  # If we got here without Twitter error, mark as successful
                    )
    ...
    async def get_user(self) -> Dict:
        ...
        try:
            ...
            if self.twitter_connected:
                logger.info(
                    f"[{self.account_index}] Twitter connected (x_id={twitter_id})"
                )
            else:
        ...
        try:
            ...
            try:
                ...
                if not self.config.DUSTED.SKIP_TWITTER_VERIFICATION:
                    if (
                        not self.twitter_connected
                        and self.twitter_token
                        and self.twitter_token.strip()
                    ):
                        logger.info(
                            f"[{self.account_index}] Twitter not connected. Attempting to connect..."
                        )
                        await self.connect_twitter()
                    elif not self.twitter_token or not self.twitter_token.strip():
                        logger.warning(
                            f"[{self.account_index}] No valid Twitter token provided. Skipping Twitter connection."
                        )
                    else:
                        logger.info(
                            f"[{self.account_index}] Twitter already connected. Skipping connection step."
                        )
                else:
                    logger.info(
                        f"[{self.account_index}] Twitter verification is disabled. Skipping Twitter connection."
                    )
                ...
                if hasattr(self, "account") and hasattr(self.account, "address"):
                    ...
                    logger.info(
                        f"[{self.account_index}] Recorded account execution time for {self.account.address}"
                    )
                ...
                logger.error(
                    f"[{self.account_index}] Error during login or subsequent steps: {e}"
                )
                return False
